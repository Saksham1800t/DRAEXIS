        ...
        <div className='mt-3'>
            ...
            <div className='mt-2'>
                {course?.courseOutput?.Chapters
                    .map((chapter, index) => (
                        <div key={index} className='border p-5 rounded-lg my-2 flex items-center justify-between'>
                            <div className='flex gap-5 items-center'>
                                <h2 className='bg-blue-500 flex-none h-10 w-10 text-white rounded-s text-center p-2'>{index + 1}</h2>
                                <div>
                                    <h2 className='font-medium text-lg'>{chapter.ChapterName}{edit && <EditChapters course={course} index={index} refreshData={refreshData} />}</h2>
